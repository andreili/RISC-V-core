...
// --------------------------------------------------------------------------
    ...
    input   wire                        i_rxd,      // Serial input
    output  wire                        o_txd,      // Transmit data output
    output  wire                        o_txen      // Transmit enabled
); // Combined interrupt
...
assign  write_enable08 = write_enable & (i_wb_adr[11:2] == 10'h002);
assign  write_enable0c = write_enable & (i_wb_adr[11:2] == 10'h003);
assign  write_enable10 = write_enable & (i_wb_adr[11:2] == 10'h004);
...
// Read operation
 ...
 always @(i_wb_adr or reg_rx_buf or uart_status or reg_ctrl or intr_state or reg_baud_div)
  begin
   if (i_wb_adr[11:5] == 7'h00) begin
     case (i_wb_adr[4:2])
  ...
  assign o_wb_dat[31: 0] = (read_enable) ? read_mux_word : {32{1'b0}};
  assign o_wb_ack  = i_dev_sel; // Always ready
...
// Transmit
  ...
  // Connect to external
  assign o_txd  = reg_txd;
  assign o_txen = reg_ctrl[0];
...
// Receive
  ...
  assign rx_data_read   = (i_dev_sel & (~i_wb_cyc) & (i_wb_adr[11:2]==10'h000) & (~i_wb_we));
...
// Interrupts
  ...
  assign intr_stat_clear[1:0] = {2{write_enable0c}} & i_wb_dat[1:0];
...
`include "std_ovl_defines.h"
   ...
   always@(posedge i_clk)
     ...
     if (~i_reset_n)
       ...
       ovl_last_psel    <= i_dev_sel;
       ovl_last_penable <= i_wb_cyc;
       ovl_last_pwrite  <= i_wb_we;
       ovl_last_paddr   <= i_wb_adr;
       ovl_last_pwdata  <= i_wb_dat;
   ...
   // Ensure tx_buf_full is set to 1 after write to TX buffer (i_wb_adr[11:2]==0)
   assert_next
     #(`OVL_ERROR, 1,1,0, `OVL_ASSERT,
      "tx_buf_full should be asserted after write to TX buffer")
   u_ovl_tx_buf_full
    (.clk(i_clk),  .reset_n(i_reset_n),
     .start_event (i_dev_sel & (~i_wb_cyc) & i_wb_we & (i_wb_adr[11:2] == 10'h000)),
   ...
   // if write to INTCLEAR  with bit[2]=1, reg_tx_overrun will be cleared,
    ...
    u_ovl_tx_buffer_overrun_clear_a
      (.clk(i_clk ), .reset_n (i_reset_n),
     .start_event (write_enable0c & (i_wb_dat[2])),
   ...
   // if write to STATUS  with bit[2]=1, reg_tx_overrun will be cleared,
    ...
    u_ovl_tx_buffer_overrun_clear_b
      (.clk(i_clk ), .reset_n (i_reset_n),
     .start_event (write_enable04 & (i_wb_dat[2])),
     .test_expr   (reg_tx_overrun==1'b0)
     );
   ...
   // if write to INTCLEAR  with bit[3]=1, reg_rx_overrun will be cleared, unless a new overrun take place
    ...
    u_ovl_rx_buffer_overrun_clear_a
      (.clk(i_clk ), .reset_n (i_reset_n),
     .start_event (write_enable0c & (i_wb_dat[3]) & (~(rx_buf_full & (rx_state==9) & (nxt_rx_state==10)))),
   ...
   // Reading of reg_baud_div (worth checking due to two stage read mux)
    ...
    u_ovl_read_baud_rate_divide_cfg
      (.clk(i_clk ), .reset_n (i_reset_n),
     .start_event   (i_dev_sel & (~i_wb_cyc) & (~i_wb_we) & (i_wb_adr[11:2]==10'h004)),
     .test_expr     (o_wb_dat=={{12{1'b0}}, reg_baud_div})
     );
